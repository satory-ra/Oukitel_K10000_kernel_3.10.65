...
#define SPI_RESET_PIN  			GPIO_FIGERPRINT_RST		//(GPIO63 | 0x80000000)
//#define GPIO_FIGERPRINT_INT 	GPIO98
...
struct cdev spicdev;
static struct spi_device *g_client_gsl6162;
...
static uint32_t gsl6162_read(struct spi_device *spi,uint8_t addr);
...
struct mt_chip_conf chip_config = {
		...
		.high_time =8,//4,//6,//8,//12, //25,//8,      //10--6m   15--4m   20--3m  30--2m  [ 60--1m 120--0.5m  300--0.2m]
		.low_time = 8,//4,//6,//8,//12,//25,//8,
...
static ssize_t gsl_sysfs_version_store(struct device *dev,struct device_attribute *attr, const char *buf,size_t count)
{	
	if(buf[0] == '1'){
		version_flag = 1;	
	}else if(buf[0] == '0'){
...
static ssize_t gsl_sysfs_version_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if(version_flag == 1)
	{
		...
		{
			msleep(20);			
			D_read = gsl6162_read(g_client_gsl6162,0xfc);	
			printk("gsl6162_read::0xfc = %08x\n",D_read);
			//ret = sl_fp_read(spidev,0xfc,r_buf,4);
			//printk("0xfc = %02x%02x%02x%02x,ret = %d\n",r_buf[3],r_buf[2],r_buf[1],r_buf[0],ret);
			msleep(100);
		}
...
static DEVICE_ATTR(version, 0666, gsl_sysfs_version_show, gsl_sysfs_version_store);
...
static uint32_t gsl6162_read(struct spi_device *spi,uint8_t addr)
{
	...
	input = kzalloc(8, GFP_KERNEL);
	output = kzalloc(8, GFP_KERNEL);
	req = kzalloc(sizeof *req, GFP_KERNEL);

	input[0] =addr;// rotate_bit(addr, 8);		// 8-bit address
	input[1] = 0x00;					// Read command
	/* read data */
	...
	//printk("gsl6162_read: %02x %02x %02x %02x %02x %02x %02x %02x\n",output[7],output[6],output[5],output[4],output[3],output[2],output[1],output[0]);
	if(rd_flag == 1 || addr  == 0x7c){
		Rd_Data = ( (output[3] << 24) | (output[4] << 16) | (output[5] << 8) | output[6] );
		rd_flag = 0;
	}else{
		Rd_Data = ( (output[2] << 24) | (output[3] << 16) | (output[4] << 8) | output[5] );
	}
	...
	return  Rd_Data;//rotate_bit(Rd_Data, 32);
...
{
	// hw reset 
	gpio_direction_output(spidev->hw_reset_gpio,1);
	...
	dev_info(&spidev->spi->dev, "shutdown silead hw\n");
	return 0;
}
static long
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_HW_IRQ_ENBALE:
		...
		else
		{
			//polling mode
			printk("polling mode");
			//disable_irq_nosync(fp_spidev->irq);									
			//mt_eint_mask(spidev->irq); 
		}
		retval = 0;
		break;
...
#endif
	sysfs_create_group(&spi->dev.kobj,&gsl_attr_group);
#if 1
...
#endif 
    spidev->irq=mt_gpio_to_irq(CUST_EINT_FIGERPRINT_INT_NUM);
    //printk("spidev->irq=%d \n",spidev->irq);
        ret = request_irq(spidev->irq, spidev_irq_routing,
				IRQF_TRIGGER_RISING, input_name, spidev);
