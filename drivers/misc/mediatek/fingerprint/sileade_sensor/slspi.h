...
#define SPI_SPEED			(5 * 1000 * 1000) //(8 * 1024 * 1024) //origin
#define SPI_BITS				8
#define SPI_DELAY			0
#define SPI_BUF_SIZE	 4096	

//#define LSB_TO_MSB
#define TRG_DEBUG "[FINGERP]"
#ifdef TRG_DEBUG
#define FP_DBUG(fmt, args...) \
	do{\
		printk(KERN_INFO TRG_DEBUG"[line:%d]"fmt"\n", __LINE__, ##args);\
	}while(0)
#define FP_ERR(fmt, args...) \
	do{\
		printk(KERN_ERR TRG_DEBUG"[line:%d]"fmt"\n", __LINE__, ##args);\
	}while(0)
#else
#define FP_DBUG(fmt, args...)
#define FP_ERR(fmt, args...)
#endif
//#define CUST_EINT_FINGER_NUM GPIO_FIGERPRINT_INT //GPIO_FINGERPRINT_EINT_PIN 
struct spidev_data {
    dev_t			devt;
    spinlock_t		spi_lock;
