...
inline static int moveAF(unsigned long a_u4Position)
{
    ...
    //printk(" AF move [curr] %ld [target] %ld\n", g_u4CurrPosition, g_u4TargetPosition);
...
static int ZC533_Init()
{

		int  i4RetValue = 0;
		char pBuff[3] = {0x05, 0x05, 0x64};
   	g_pstAF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
    i4RetValue = i2c_master_send(g_pstAF_I2Cclient, pBuff, 3);
    if (i4RetValue < 0) 
    {
        LOG_INF("[AF] I2C send failed!! \n");
        return -1;
