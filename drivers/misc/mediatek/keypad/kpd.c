...
int hall_fun_status = 1;

static ssize_t kpd_store_hall_fun_state(struct device_driver *ddri, const char *buf, size_t count)
{
	if (sscanf(buf, "%d", &hall_fun_status) != 1) {
		kpd_print("hall_fun_status state: Invalid values\n");
		return -EINVAL;
	}
...
static ssize_t kpd_show_hall_fun_state(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	res = snprintf(buf, PAGE_SIZE, "%d\n", hall_fun_status);
	return res;
...
static DRIVER_ATTR(kpd_hall_fun_state, 0x666, kpd_show_hall_fun_state, kpd_store_hall_fun_state);

#endif
static struct driver_attribute *kpd_attr_list[] = {
	&driver_attr_kpd_call_state,
	#ifdef CONFIG_HCT_HALL_SUPPORT
	&driver_attr_kpd_hall_fun_state,
	#endif
};
