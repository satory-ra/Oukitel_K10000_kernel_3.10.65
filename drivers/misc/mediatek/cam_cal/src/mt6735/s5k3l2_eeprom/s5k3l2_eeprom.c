...
#define PFX "S5K3L2_EEPROM"
#ifdef CAM_CAL_DEBUG

#define CAM_CALINF(fmt, arg...)     pr_info("[%s] " fmt, __FUNCTION__, ##arg) //xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " fmt, __FUNCTION__, ##arg)
#define CAM_CALDB(fmt, arg...)      pr_info("[%s] " fmt, __FUNCTION__, ##arg)
#define CAM_CALERR(fmt, arg...)      pr_info("[%s] " fmt, __FUNCTION__, ##arg)
#else
#define CAM_CALINF(x,...)
#define CAM_CALDB(x,...)
...
#define MAX_LSC_SIZE 1124
#define MAX_OTP_SIZE 1139
static int s5k3l2_eeprom_read = 0;

typedef struct {
        u8     flag;
	u16    Modeule_lens_id;//0xff000b01
	u8     year;
	u8     month;
	u8     day;
	u16    vcm_driver_id;//0x06
	u8     UnitAwb_R_Gr_h;
	u8     UnitAwb_R_Gr_l;
	u8     UnitAwb_B_Gr_h;
	u8     UnitAwb_B_Gr_l;
	u8     GoldenAwbGr_h;
       u8     GoldenAwbGr_l;
#if 0   // Jay: this two byte need used as lsc size by platform. awb no used
        u16     awbChecksum;
#else
      u16    LscSize;             //0x0E
#endif
	u8   Lsc[MAX_LSC_SIZE];   // 0x10
}OTP_MTK_TYPE;
...
int read_s5k3l2_eeprom(u8 slv_id, u16 offset, u8* data)
{
    ...
    //CAM_CALINF("OTP read slv_id 0x%x offset 0x%x  data 0x%x\n", slv_id,offset,*data);
...
#if 0
    read_s5k3l2_eeprom_size(0xA0,0x00,&s5k3l2_eeprom_data.Data[0x00],1);
#endif

	//read calibration version 0xff000b01
       ret=read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x00,&s5k3l2_eeprom_data.Data[0x00],1);
      // CAM_CALINF("read_s5k3l2_eeprom_mtk_fmt =%d \n",ret);
	if( ret!= 0x00)
	{
                s5k3l2_eeprom_read = -1;
                CAM_CALINF("read s5k3l2_eeprom  i2c fail !!!! \n");
                return -1;
	}

        if(s5k3l2_eeprom_data.Data[0x00] !=0x01)
         {
                s5k3l2_eeprom_read = -1;
                CAM_CALINF("read s5k3l2_eeprom  module data is invalid, data=%x \n",s5k3l2_eeprom_data.Data[0x00]  );
                return -1;
         }

	//read Modeule_lens_id
	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x01,&s5k3l2_eeprom_data.Data[0x01],2);

          CAM_CALINF("OTP readed =mode_id= %x \n",s5k3l2_eeprom_data.Data[0x01]);
          
          CAM_CALINF("OTP readed =lens_id= %x \n",s5k3l2_eeprom_data.Data[0x02]);
	//read Vcm_Driver_id
	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x06,&s5k3l2_eeprom_data.Data[0x06],2);

        s5k3l2_eeprom_data.Data[0x0E]= ((1124)&0xFF00)>>8; // thi store lsc data lenth
    
       s5k3l2_eeprom_data.Data[0x0F]= (1124&0xFF); // thi store lsc data lenth
#if 0
	//read AF config
	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x07,&s5k3l2_eeprom_data.Data[0x07],2);

	//read AWB
	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x09,&s5k3l2_eeprom_data.Data[0x09],8);

	//read AF calibration
	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x11,&s5k3l2_eeprom_data.Data[0x011],4);
#endif
    //read LSC size
//	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x15,&s5k3l2_eeprom_data.Data[0x015],2);
#if 0
    int size = 0;
        size = s5k3l2_eeprom_data.Data[0x015]+s5k3l2_eeprom_data.Data[0x016]<<4;
#endif


    //for lsc data
   	read_s5k3l2_eeprom_size(s5k_eeprom_addr,0x10,&s5k3l2_eeprom_data.Data[0x10],(1124));
#if 0
	...
	read_s5k3l2_eeprom_size(0xA6,0x00,&s5k3l2_eeprom_data.Data[offset],0xBA-0+1);
	CAM_CALDB("final offset offset %d ! \n",offset+0xBA);
        CAM_CALDB("size %d readed %d! \n",size,offset+0xBA-0x17+1);
		u8 data[9];
		read_s5k3l2_eeprom_size(0xAA,0xE0,&data[0],8);
#endif
	spin_lock(&g_CAM_CALLock);
	s5k3l2_eeprom_read = 1;
	spin_unlock(&g_CAM_CALLock);
