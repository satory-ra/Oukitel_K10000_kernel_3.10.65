...
#if defined(CONFIG_HCT_LCM_ESD_SUPPORT_LONG_PACKET)
static UINT32 data_paras[2] = {0x05,0x21};  //add himax fae for 0x45
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
        DSI_T0_INS t1;
	#if defined(CONFIG_T825A_DH_PROJ)
	DSI_T0_INS t2;
	#endif
	DSI_RX_DATA_REG read_data0;
 DSI_RX_DATA_REG read_data1;
 ...
 static cmdqBackupSlotHandle hSlot[4] = {0, 0, 0, 0};
	...
	{	
		...
		t2.Data_ID = 0x02;
		//mtk_add
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[0], AS_UINT32(&t2));
			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[1], AS_UINT32(&t1));

			DSI_OUTREG32(cmdq_trigger_handle, &DSI_CMDQ_REG[dsi_i]->data[2], AS_UINT32(&t0));

			DSI_OUTREG32(cmdq_trigger_handle, &DSI_REG[dsi_i]->DSI_CMDQ_SIZE, 3);
#else
                       // write DSI CMDQ
	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			if(packet_type == 0x1A || packet_type == 0x1C)
			{
				...
				DISPCHECK("hct esd: DSI read long packet size: %d %d %d = %d %d %d\n", read_data1.byte0,read_data1.byte1,read_data1.byte2, dsi_params->lcm_esd_check_table[i].para_list[0],dsi_params->lcm_esd_check_table[i].para_list[1],dsi_params->lcm_esd_check_table[i].para_list[2]);
...
#elif defined(CONFIG_HX8394_ESD_OPEN)
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	int ret = 0;
	int i   = 0;
