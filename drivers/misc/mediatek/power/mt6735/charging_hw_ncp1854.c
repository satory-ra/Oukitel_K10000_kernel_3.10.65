...
//  ncp1854_set_int_mask(0x0); //disable all interrupt
   ...
   ncp1854_set_chgto_dis(0x1); //disable charge timer
	...
	#if defined(CONFIG_T823T_HS_PROJ)||defined(CONFIG_T823K_CQ_PROJ)||defined(CONFIG_T823AB_CQ_PROJ)
		ncp1854_set_ctrl_vbat(0x2A);
	#else
...
ncpreg_val     terminate current  for ICS optimized suspend power
    0x00           100ma
	0x01           125ma
    0x02           150ma
    0x03           175ma
...
*/
	...
	#else
		ncp1854_set_ieoc(0x3); // terminate current = 175mA for ICS optimized suspend power
	#endif
#else
	#if defined(CONFIG_HCT_NCP_HIGHER_CV)
			ncp1854_set_ieoc(0x4); // terminate current = 200mA for ICS optimized suspend power
  	#else
		#if defined(CONFIG_T823T_HS_PROJ)||defined(CONFIG_T985C_CH)
			ncp1854_set_ieoc(0x4);
		#else
        	ncp1854_set_ieoc(0x2); // terminate current = 125mA for ICS optimized suspend power
		#endif
  	#endif
#endif
 ...
 static kal_uint32 charging_set_current(void *data)
 {
	...
	kal_uint32 current_value = *(kal_uint32 *)data;
      kal_uint32 charg_enable =KAL_FALSE;
	//kal_uint32 current_high_flag = 0;
      kal_uint32  sub_ncp_enable = KAL_TRUE;
	...
	ncp1854_set_ichg(register_value);       	
       ...
       if(sub_current_value)
        {
           sub_ncp_enable = KAL_TRUE;
           ncp1854_charging_enable_sub(&sub_ncp_enable);
            charging_set_current_sub(sub_current_value);
        }
        else
        {
            sub_ncp_enable = KAL_FALSE;
            ncp1854_charging_enable_sub(&sub_ncp_enable);
        }
#endif
