...
static int ALS_INTT				= EPL_INTT_ALS_150;
...
static int GES_INTT				= EPL_INTT_ALS_250;
...
#if DYN_ENABLE
static void dyn_ps_cal(struct epl8865_priv *epl_data)
{
...
static int epl8865_psensor_enable(struct epl8865_priv *epl_data, int enable)
{
    ...
    if(enable)
    {
        ...
        epl8865_I2C_Write(client,REG_14,R_TWO_BYTE,0x01,0x00);
	    epl8865_I2C_Read(client);
...
static void epl8865_gesture_rawdata(void)
{
    ...
    if(GESTURE_DEBUG == 3)
    {
        epl8865_I2C_Write(client,REG_13,R_SINGLE_BYTE,0x01,0);
        epl8865_I2C_Read(client);
        ges_mode = (gRawData.raw_bytes[0]>>3)&7;
        GES_LOG("[%s]: ges_mode = %d \n\n", __func__, ges_mode);
        if(ges_mode != (EPL_GES_MODE >> 4))
        {
            GES_LOG("[%s]: Failed! Wrong mode..............\r\n", __func__);
        }
    }
...
void epl8865_polling_work(struct work_struct *work)
{
    ...
    else if(enable_ges)
    {
        ...
        else
        {
            ...
            epl8865_gesture_enable(obj, false, ges_frame==0?true:false);
	        schedule_delayed_work(&obj->polling_work, msecs_to_jiffies(10));
        }
    }
...
static void epl8865_eint_work(struct work_struct *work)
{
    ...
    else if(test_bit(CMC_BIT_PS, &epld->enable))
    {
        ...
        if(epld->pending_intr)
        {
            ...
            if(err != 0)
            {
                APS_ERR("epl8865_eint_work err: %d\n", err);
            }
#else
    ...
    {
        epl8865_gesture_rawdata();
		epl8865_gesture_enable(epld, true, ges_frame==0?true:false);
    }
...
#if MTK_LTE
    if((err = epl8865_create_attr(&epl_sensor_init_info.platform_diver_addr->driver)))
    {
...
static int epl8865_i2c_remove(struct i2c_client *client)
{
    int err;
#if MTK_LTE
    if((err = epl8865_delete_attr(&epl_sensor_init_info.platform_diver_addr->driver)))
    {
