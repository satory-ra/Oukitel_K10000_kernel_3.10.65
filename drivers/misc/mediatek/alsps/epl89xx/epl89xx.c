/* drivers/hwmon/mt6516/amit/epl89xx.c - EPL89xx ALS/PS driver
 *
...
#if 1 //def CONFIG_MTK_AUTO_DETECT_ALSPS
#define MTK_LTE         1   //4G
#else
#define MTK_LTE         0   //3G
#endif
...
static int als_dynamic_intt_intt[] = {EPL_ALS_INTT_1000, EPL_ALS_INTT_70};
static int als_dynamic_intt_value[] = {1000, 70};
...
static void write_global_variable(struct i2c_client *client)
{
...
#endif
    {
        ...
        epl_sensor_I2C_Write(client, 0x24, buf);
        ...
        epl_sensor_I2C_Write(client, 0x25, buf);
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    dynk_enhance_integration_time = EPL_ALS_INTT_70;
    dynk_enhance_gain = EPL_GAIN_LOW;
    dynk_enhance_adc = EPL_PSALS_ADC_11;
#endif
#endif
    //ps factory
    epl_sensor.ps.factory.calibration_enable =  false;
    ...
    //ges setting
    epl_sensor.ges.integration_time = EPL_GES_INTT_80;
    ...
    epl_sensor.ges.polling_mode = false;
    ...
    epl_sensor.ges.ir_boost = EPL_IR_BOOST_300;
...
static void epl_sensor_notify_event(int event)
{
    struct input_dev *idev = epl_sensor_obj->gs_input_dev;
    APS_LOG("notify event %d\n",event);
...
static void epl_sensor_gesture_rawdata(void)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    ...
    epl_sensor_I2C_Read(obj->client, 0x27, 6*index);
    ...
    {
        epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait);
		epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait | epl_sensor.mode);
    ...
    {
		epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait_sleep);
        epl_sensor_I2C_Write(obj->client, 0x00, epl_sensor.ges.wait_sleep| epl_sensor.mode);
        sleep_mode=1;
...
void epl_sensor_polling_work(struct work_struct *work)
{
...
#endif
#if GES_DEBUG
    APS_LOG("als / ps / gesture enable: %d / %d / %d\n", enable_als, enable_ps ,enable_ges);
#endif
    cancel_delayed_work(&obj->polling_work);
    ...
    else if(enable_ges && epl_sensor.ges.polling_mode)
#else
    if(enable_ges && epl_sensor.ges.polling_mode)
#endif
    {
        mutex_lock(&sensor_mutex);
        epl_sensor_gesture_rawdata();
        ...
        schedule_delayed_work(&obj->polling_work, msecs_to_jiffies(sleep_mode==1?20:10));
    }
}
...
void epl_sensor_enhance_enable(struct i2c_client *client, bool enable)
{
	...
	if(now_cmp_l != epl_sensor.ps.compare_low && eint_flag == true)
	{
    ...
    if(enable == true){
        ...
        epl_sensor_I2C_Write(client, 0xfc, epl_sensor.osc_sel| EPL_GFIN_ENABLE | EPL_VOS_ENABLE | EPL_DOC_OFF);
    }
    else
    {
...
void epl_sensor_update_mode(struct i2c_client *client)
{
...
#endif
    {
        ...
        //if ges and als are enabled, setting 11 ADC in als
        if(enable_ges && enable_als)
        {
            epl_sensor_I2C_Write(client, 0x02, EPL_PSALS_ADC_11 | epl_sensor.als.cycle);
            set_als_ps_intr_type(client, epl_sensor.ps.polling_mode, 1);    //if ges and als enable, als is only support polling
        }
        else
        {
            epl_sensor_I2C_Write(client, 0x02, epl_sensor.als.adc | epl_sensor.als.cycle);
        ...
        epl_sensor_I2C_Write(obj->client, 0x25, epl_sensor.ges.interrupt_enable | epl_sensor.ges.interrupt_index | epl_sensor.ges.ir_mode | epl_sensor.ges.ir_boost | epl_sensor.ges.ir_drive);
        ...
        if(enable_ges == 1 && epl_sensor.ges.polling_mode == 0)
        {
#if defined(MT6575) || defined(MT6571) || defined(MT6589)
            mt65xx_eint_unmask(CUST_EINT_ALS_NUM);
#else
    ...
    if(epl_sensor.ges.polling_mode && enable_ges)
        epl_sensor_restart_polling();
...
static void epl_sensor_eint_work(struct work_struct *work)
{
    ...
    if(enable_ges && enable_ps == 0)
    {
        ...
        if(enable_als)
        {
            epl_sensor.als.report_count = epl_sensor.als.report_count + (1<<(epl_sensor.ges.interrupt_index>>5));
            if(epl_sensor.als.report_count >= GES_ALS_COUNT)
            {
                epl_sensor_read_als(obj->client);
                epl_sensor.als.report_count = 0;
        ...
        if(epl_sensor.ges.wait == EPL_WAIT_SINGLE)
        {
            ...
            {
                epl_sensor_I2C_Write(obj->client, 0x24, epl_sensor.ges.adc | epl_sensor.ges.integration_time |epl_sensor.ges.gain);
                sleep_mode=0;
                sleep_reset=0;
            ...
            {
                epl_sensor_I2C_Write(obj->client, 0x24, EPL_GES_ADC_14 | epl_sensor.ges.integration_time |epl_sensor.ges.gain);
                sleep_mode=1;
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_show_ges_raws(struct device_driver *ddri, char *buf)
{
    u16 *tmp = (u16*)buf;
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_int_time(struct device_driver *ddri, const char *buf, size_t count)
{
    struct epl_sensor_priv *obj = epl_sensor_obj;
    int val;
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_opt(struct device_driver *ddri, const char *buf, size_t count)
{
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_int_filter_order(struct device_driver *ddri, const char *buf, size_t count)
{
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_debug(struct device_driver *ddri, const char *buf, size_t count)
{
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_work_threshold(struct device_driver *ddri, const char *buf, size_t count)
{
...
static ssize_t epl_sensor_store_gain(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	switch (epl_sensor.mode)
	{
		...
		case EPL_MODE_GES: //ges
	        ...
	        epl_sensor_I2C_Write(epld->client, 0x24, epl_sensor.ges.integration_time | epl_sensor.ges.gain);
		break;
    }
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_mode(struct device_driver *ddri, const char *buf, size_t count)
{
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_zoom_delta(struct device_driver *ddri, const char *buf, size_t count)
{
...
/*----------------------------------------------------------------------------*/
static ssize_t epl_sensor_store_ges_index(struct device_driver *ddri, const char *buf, size_t count)
{
    int value=0;
    uint8_t  data;
    struct epl_sensor_priv *obj = epl_sensor_obj;
    ...
    APS_LOG("%s: 0x25 = 0x%x\n", __FUNCTION__, data);
    epl_sensor_I2C_Write(obj->client, 0x25, data);
    epl_sensor_update_mode(obj->client);
...
static ssize_t epl_sensor_store_hs_int_time(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	mutex_lock(&hs_sensor_mutex);
	epl_sensor.hs.integration_time = value<<1;
...
static ssize_t epl_sensor_show_sen(struct device_driver *ddri, char *buf)
{
    ...
    APS_FUN();
#if 0// !(defined(MT6582) || defined(MT6592) || defined(MT6735) || defined(MT6752) || defined(MT6753))
...
static DRIVER_ATTR(ges_raws, 					S_IROTH  | S_IWOTH, epl_sensor_show_ges_raws, 	  		NULL										);
static DRIVER_ATTR(ges_enable,					S_IROTH  | S_IWOTH, NULL,   					 		epl_sensor_store_ges_enable					);
static DRIVER_ATTR(ges_mode,				    S_IROTH  | S_IWOTH, NULL,								epl_sensor_store_ges_mode					);
static DRIVER_ATTR(ges_int_time,				S_IROTH  | S_IWOTH, NULL,   					 		epl_sensor_store_ges_int_time					);
static DRIVER_ATTR(ges_polling_mode,			S_IROTH  | S_IWOTH, NULL,   					 		epl_sensor_store_ges_polling_mode				);
static DRIVER_ATTR(ges_opt,					    S_IROTH  | S_IWOTH, NULL,   					 		epl_sensor_store_ges_opt						);
static DRIVER_ATTR(ges_int_filter_order,		S_IROTH  | S_IWOTH, NULL,   							epl_sensor_store_ges_int_filter_order			);
static DRIVER_ATTR(ges_debug,					S_IROTH  | S_IWOTH, NULL,   					 		epl_sensor_store_ges_debug					);
static DRIVER_ATTR(ges_work_threshold,			S_IROTH  | S_IWOTH, NULL,   							epl_sensor_store_ges_work_threshold			);
static DRIVER_ATTR(ges_zoom_delta,				S_IROTH  | S_IWOTH, NULL,   							epl_sensor_store_ges_zoom_delta				);
static DRIVER_ATTR(ges_index,					S_IROTH  | S_IWOTH, NULL,								epl_sensor_store_ges_index					);
...
static struct driver_attribute * epl_sensor_attr_list[] =
{
    ...
    &driver_attr_ges_opt,
    &driver_attr_ges_int_filter_order,
...
static int epl_sensor_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(obj->hw->polling_mode_ps == 0 || obj->hw->polling_mode_als == 0 || !epl_sensor.ges.polling_mode)
...
MODULE_DESCRIPTION("EPL89xx ALPsr driver");
MODULE_LICENSE("GPL");
