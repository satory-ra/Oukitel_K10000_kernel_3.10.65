...
#define KEYCODE_ZOOM_L1		KEY_F7
#define KEYCODE_ZOOM_L2		KEY_F8
#define KEYCODE_ZOOM_L3		KEY_F9
...
#define GES_ZOOM_START_TIME		400  //400ms
#define GES_ZOOM_SPACING_TIME		200 	//400ms

#define GES_MODE_HORIZONTAL		2
#define GES_MODE_VERTICAL		       3
#define GES_MODE_4D				4
#define GES_MODE_6D				6
...
static bool is_zoom_triger   =false;
...
static struct timeval zoom_triger_time;

static int zoom_delta=12;
static int zoom_last_level;
...
static int set_gesture_mode(int gm)
{
    ges_mode = gm;
    if(gm==1)
        ges_mode = GES_MODE_4D;
...
static int get_gesture_event(void)
{
    ...
    else if(ges_mode== GES_MODE_4D || ges_mode== GES_MODE_6D)
...
static void update_crosstalk(int cmp_data)
{
    ...
    if (ges_mode == GES_MODE_6D)
        return;

    if(crosstalk_last <= cmp_data+crosstalk_offset && crosstalk_last >= cmp_data-crosstalk_offset)
    {
        crosstalk_count++;
...
static int detect_zoom_event(int cmp_data)
{
    bool is_zoom;
    ...
    is_zoom = cmp_data > ZOOM_TH;

    if (!is_zoom)
    {
        is_zoom_triger = false;
        is_zoom_notify = false;
        return evt;
    }

    zoom_new_level = (int)(int_sqrt(cmp_data) / zoom_delta);
    zoom_new_level = zoom_new_level > 2 ? 2 : zoom_new_level;
    zoom_new_level += EVENT_ZOOM_L1;

    if (!is_zoom_triger)
    {
        is_zoom_notify = false;
        is_zoom_triger = true;
        do_gettimeofday(&zoom_triger_time);
        zoom_last_level = zoom_new_level;
        return evt;
    }

    if (zoom_new_level != zoom_last_level)
    {
        do_gettimeofday(&zoom_triger_time);
        zoom_last_level = zoom_new_level;
    }

    if (is_zoom_notify == false && get_time_diff(&zoom_triger_time) > GES_ZOOM_START_TIME)
    {
        is_zoom_notify = true;
        evt = zoom_new_level;
        do_gettimeofday(&zoom_triger_time);
    ...
    else if (is_zoom_notify == true && get_time_diff(&zoom_triger_time) > GES_ZOOM_SPACING_TIME)
    {
        evt = zoom_new_level;
        do_gettimeofday(&zoom_triger_time);
        zoom_last_level = zoom_new_level;
...
static int detect_gesture_event(void)
{
    ...
    evt = detect_zoom_event(cmp_data);
    ...
    else if(isNotWork)
    {
        if(is_work_triger && is_event_triger != STATUS_RESTART)
        {
            ...
            {
                GES_LOG("event_time %d ms out of range\n",event_time);
            }
        }
