...
#define PS_GES          0
#if defined(CONFIG_EPL_PS_DYN_K_SUPPORT)
#define PS_DYN_K        1  // this is dynamic cal, default need to close, 
#else
#define PS_DYN_K        0  // this is dynamic cal, default need to close, 
#endif
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
	...
	#if defined(CONFIG_T87_PROJ)||defined(CONFIG_T875A_T92_PROJ)||defined(CONFIG_T875F_T95_PROJ)||defined(CONFIG_T875K_S02_PROJ)
	epl_sensor.ps.integration_time = EPL_PS_INTT_784;//80;lfr
	#else
...
#if ALS_DEBUG
    APS_LOG("[%s]:raw_data=%d, epl_sensor.als.dyn_intt_raw=%d, lux=%ld\r\n", __func__, raw_data, epl_sensor.als.dyn_intt_raw, (long int)lux);
#endif
...
/*----------------------------------for factory mode test---------------------------------------*/
		...
		case ALSPS_IOCTL_CLR_CALI:
            ...
            APS_LOG("HCT_LFR [%s]: ALSPS_IOCTL_CLR_CALI \r\n", __func__);
#endif
			break;
...
#else
            APS_LOG("HCT_LFR [%s]: ALSPS_IOCTL_GET_CALI \r\n", __func__);
#endif
			break;
		...
		case ALSPS_IOCTL_SET_CALI:
			...
			APS_ERR("%s set ps_calix%x\n", __func__, obj->ps_cali);
#else
            APS_LOG("HCT_LFR [%s]: ALSPS_IOCTL_SET_CALI \r\n", __func__);
#endif
			break;
...
#else
            APS_LOG("HCT_LFR [%s]: ALSPS_SET_PS_THRESHOLD \r\n", __func__);
#endif
			break;
#endif //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
