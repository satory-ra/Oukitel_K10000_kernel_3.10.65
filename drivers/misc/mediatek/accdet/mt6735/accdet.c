...
#ifdef CONFIG_HCT_SMART_KEY_DETECT_SUPPORT
#include <linux/time.h>

static struct hrtimer smart_key_kthread_timer;
 ktime_t smart_key_ktime;
 int smart_key_thread_timeout=0;

static DEFINE_MUTEX(smart_key_mutex);
static DECLARE_WAIT_QUEUE_HEAD(smart_key_thread_wq);
  ...
  typedef enum
 {
     ...
     THIRD_KEY_REPO =81,
     LONG_KEY_REPO = 82,
     INVALED_KEY_REPO = 0x4FF
 }INPORT_KEY_TYPE;

#define KEY_SHORT_PRESS_CHECK_TIME 2

#define KEY_LOG_PRESS_TIME 2
 
static SMRT_KEY_STATUE key_status=SMART_KEY_IDLE;
static KEY_STATUS current_key_status; 
static INPORT_KEY_TYPE inpurt_Key;

unsigned long down_press_tick;
unsigned long up_press_tick;
unsigned long long_press_check_tick;

static volatile int g_smart_key_enable = 0;

struct wake_lock smart_key_thread_lock;
static int smart_key_press_count =0;
static int smart_key_release_count=0;
static int smart_key_short_release_count =0;

void wake_up_smart_key(void)
 {
    smart_key_thread_timeout = 1;
     wake_up(&smart_key_thread_wq);
 }

enum hrtimer_restart smart_key_kthread_hrtimer_func(struct hrtimer *timer)
 {
...
void smart_key_timer_init(void)
 {
    smart_key_ktime = ktime_set(2,0);       // 3s, 10* 1000 ms
    hrtimer_init(&smart_key_kthread_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);

    smart_key_kthread_timer.function = smart_key_kthread_hrtimer_func;   
  //  battery_xlog_printk(BAT_LOG_CRTI, "battery_kthread_hrtimer_init : done\n" );
}

 static int smart_key_thread_kthread(void *x)
{

    /* Run on a process content */
    while (1) {
            wait_event(smart_key_thread_wq, smart_key_thread_timeout);
            
            printk("Jay_debug: thread be waked up --,release conde=%d----\n",smart_key_short_release_count);

            mutex_lock(&smart_key_mutex);

            if(inpurt_Key == LONG_KEY_REPO)
                inpurt_Key = LONG_KEY_REPO;
            else if(smart_key_short_release_count >= 3)
              inpurt_Key = THIRD_KEY_REPO ;
            else if(smart_key_short_release_count ==2)
                inpurt_Key = TWO_KEY_REPO ;
            else
              inpurt_Key = SINGLE_KEY_REPO;
              


            key_status = SMART_KEY_IDLE;
            smart_key_press_count = 0;
            smart_key_release_count = 0;

            input_report_key(kpd_accdet_dev, inpurt_Key, 1);            
            input_report_key(kpd_accdet_dev, inpurt_Key, 0);
            input_sync(kpd_accdet_dev);
            printk("Jay_debug: report smart key value=%d \n",inpurt_Key);


            smart_key_short_release_count = 0;
            smart_key_thread_timeout=0;
            inpurt_Key = INVALED_KEY_REPO;
            mutex_unlock(&smart_key_mutex);
            printk("Jay_debug: thread finished\n");
        }
    return 0;
}

void smart_key_detect_start(void)
 {
    hrtimer_start(&smart_key_kthread_timer, smart_key_ktime, HRTIMER_MODE_REL);
}

void smart_key_detect_timer_stop(void)
{
    hrtimer_cancel(&smart_key_kthread_timer);
}

static void send_smart_key_event(int flag)
{
    mutex_lock(&smart_key_mutex);

      current_key_status = (flag==1)?SMART_KEY_DOWN:SMART_KEY_UP;
      if(current_key_status == SMART_KEY_DOWN)  // this means key is pressed
      {
          down_press_tick = jiffies;
          long_press_check_tick = jiffies+KEY_LOG_PRESS_TIME * HZ;
          if(key_status ==SMART_KEY_IDLE)
         {
            printk("start_detect_smart_key-----\n");
            key_status = SMART_KEY_DETECTING;
            smart_key_detect_start();
            smart_key_press_count++;
         }
          else
          {
              smart_key_press_count++;
      ...
      {
           smart_key_release_count++;
           up_press_tick = jiffies;

           if(smart_key_release_count!= smart_key_press_count)
           {
               printk("Jay_debug: this is unuser case,need check\n");
           }

           if(time_before(jiffies, long_press_check_tick))
           {
               smart_key_short_release_count++;
           }
           else  
           {
                printk("Jay_debug: this means long key press\n");
                inpurt_Key = LONG_KEY_REPO;
                smart_key_detect_timer_stop(); 
                wake_up_smart_key();
           }

           if(key_status ==SMART_KEY_IDLE)
          {
               printk("Jay_debug: alert:this is key idle up key\n");
          }
           else
          {
              if(smart_key_thread_timeout)
                 wake_up(&smart_key_thread_wq);
          }
      }
...
static void send_key_event(int keycode,int flag)
{
	switch (keycode)
		...
		case MD_KEY:
#ifdef CONFIG_HCT_SMART_KEY_DETECT_SUPPORT
		if(g_smart_key_enable)
                        send_smart_key_event(flag);
		else{
...
static ssize_t show_smart_key_enable(struct device_driver *ddri, char *buf)
{
	int enable = 0;
	enable = g_smart_key_enable;
	ACCDET_DEBUG("ACCDET show_smart_key_enable = %d\n", enable);
	return sprintf(buf, "%u\n", enable);
}

static ssize_t store_smart_key_enable(struct device_driver *ddri, const char *buf, size_t count)
{
	int enable = 0;
	if (sscanf(buf, "%u", &enable) != 1) {
			ACCDET_DEBUG("accdet: store_smart_key_enable Invalid values\n");
			return -EINVAL;
	}
	ACCDET_DEBUG("ACCDET store_smart_key_enable = %d\n", enable);
	g_smart_key_enable = enable;
}

static DRIVER_ATTR(smart_key_enable, 0664, show_smart_key_enable, store_smart_key_enable);
#endif
...
#ifdef CONFIG_HCT_SMART_KEY_DETECT_SUPPORT	
    __set_bit(SINGLE_KEY_REPO, kpd_accdet_dev->keybit);
    __set_bit(TWO_KEY_REPO, kpd_accdet_dev->keybit);
    __set_bit(THIRD_KEY_REPO, kpd_accdet_dev->keybit);
    __set_bit(LONG_KEY_REPO, kpd_accdet_dev->keybit);
#endif
...
#ifdef CONFIG_HCT_SMART_KEY_DETECT_SUPPORT
        smart_key_timer_init();
        kthread_run(smart_key_thread_kthread, NULL, "smart_key_thread_kthread");
#endif
