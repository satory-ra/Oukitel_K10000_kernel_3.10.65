...
/**************************************************
IC Name: RM68200GA0
Panel Maker/Size: AUO495
Panel ID: H495TAN01.0
Purpose: CODE
Remark: V2.0
Version: 
Date: D:\PORJECT\F056\A4\ÂI«G¹êÅç\§ï\AUO495\RM68200GA0_AUO495_H495TAN01.0_CODE_V2.0_150119
**************************************************/
{0xFE, 1,{0x01}},
...
#define ADC_MIN_VALUE	0x68E
//0				0
//47 / 147 * 4096 = 0x51D		0.575V
//100/ 200 * 4096 = 0x800		0.9V
...
static unsigned int lcm_compare_id(void)
{
...
#if defined(BUILD_LK)
	printf("hct_rm68200_dsi_vdo_hd_auo_100k47k adc= %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#else
	printk("hct_rm68200_dsi_vdo_hd_auo_100k47k adc= %x adcdata= %x %x, ret=%d, ADC_MIN_VALUE=%x\r\n",rawdata, adcdata[0], adcdata[1],ret, ADC_MIN_VALUE);
#endif
	...
	{	 
		...
		id = (id_high<<8) | id_low;
	#if defined(BUILD_LK)
	printf("%s, [hct_rm68200_dsi_vdo_hd_auo_100k47k]  buffer[0] = [0x%x] buffer[2] = [0x%x] ID = [0x%x]\n",__func__, id_high, id_low, id);
	#else
	printk("%s, [hct_rm68200_dsi_vdo_hd_auo_100k47k]  buffer[0] = [0x%x] buffer[2] = [0x%x] ID = [0x%x]\n",__func__, id_high, id_low, id);
	#endif
...
#ifndef BUILD_LK
    printk("RM68200 lcm_esd_check enter %x\n",buffer[0]);
#endif
#ifndef BUILD_LK
        if(buffer[0] == 0x9C)
        {
...
// ---------------------------------------------------------------------------
LCM_DRIVER hct_rm68200_dsi_vdo_hd_auo_100k47k = 
{
	.name			= "hct_rm68200_dsi_vdo_hd_auo_100k47k",
