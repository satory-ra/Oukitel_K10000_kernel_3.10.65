...
******************************************************************/
 ...
 bool cust_power_on = false;
int cust_hdmi_power_on(int on)
{
	if(on > 0) 
	{
	    printk("MHL_Power power  \n" );
#ifdef GPIO_MHL_POWER_CTRL_PIN //PMIC_APP_MHL_POWER_LDO1
...
int cust_hdmi_dpi_gpio_on(int on)
{
	...
	unsigned int dpi_pin_start2 =0;
	if(on > 0)
	{
#ifdef GPIO_DPI_HSYNC_PIN
		for(dpi_pin_start = GPIO_DPI_HSYNC_PIN; dpi_pin_start < GPIO_DPI_HSYNC_PIN + 8; dpi_pin_start++)
		{
			mt_set_gpio_mode(dpi_pin_start, GPIO_MODE_02);
		}
		for(dpi_pin_start2 = GPIO_DPI_D4_PIN; dpi_pin_start2 < GPIO_DPI_D4_PIN + 8; dpi_pin_start2++)
		{
			mt_set_gpio_mode(dpi_pin_start2, GPIO_MODE_02);
		}
		printk("%s, %d GPIO_EXT_DISP_DPI0_PIN is defined+ %x\n", __func__, __LINE__, GPIO_DPI_HSYNC_PIN);
#else
	...
	else
	{
		...
		for(dpi_pin_start2 = GPIO_DPI_D4_PIN; dpi_pin_start2 < GPIO_DPI_D4_PIN + 8; dpi_pin_start2++)
		{
			mt_set_gpio_mode(dpi_pin_start2, GPIO_MODE_00);
			mt_set_gpio_dir(dpi_pin_start2, GPIO_DIR_IN);
			mt_set_gpio_pull_enable(dpi_pin_start2, GPIO_PULL_ENABLE);
			mt_set_gpio_pull_select(dpi_pin_start2, GPIO_PULL_DOWN);
		}
		printk("%s, %d GPIO_EXT_DISP_DPI0_PIN is defined- %x\n", __func__, __LINE__, GPIO_DPI_D4_PIN);
#endif
	}
