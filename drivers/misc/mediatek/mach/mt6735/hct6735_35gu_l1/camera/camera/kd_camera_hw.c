...
BOOL hwpoweron(PowerInformation pwInfo, char* mode_name)
{
	...
	else if(pwInfo.PowerType==SensorMCLK)
	{
		...
		{
			PK_DBG("Sensor MCLK1 On");
			#if defined(CONFIG_T93_PROJ) || defined(CONFIG_T875_PROJ) || defined(CONFIG_T991_PROJ) || defined(CONFIG_T925_PROJ)||defined(CONFIG_T823_PROJ)
				#if defined(CONFIG_T93K_PROJ)||defined(CONFIG_T935K_PROJ)
					ISP_MCLK1_EN(FALSE);
					mdelay(5);
				#endif
					...
					if(PowerOnList.PowerSeq[pwListIdx].PowerInfo[pwIdx].PowerType != VDD_None)
					{
						...
						if(PowerOnList.PowerSeq[pwListIdx].PowerInfo[pwIdx].PowerType == PDN)
						{
							if(pinSetIdx == 0)
							{
								...
								{
									printk("hsl camera test\n");
									mt_set_gpio_out(CAMERA_CMRST1_PIN,1);
									mt_set_gpio_out(CAMERA_CMPDN1_PIN,1);
									mdelay(10);
									mt_set_gpio_out(CAMERA_CMRST1_PIN,0);
									mt_set_gpio_out(CAMERA_CMPDN1_PIN,0);
									mdelay(10);
								}
							}
						}
