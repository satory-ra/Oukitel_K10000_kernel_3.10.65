...
#include "msg28xx_liansi_update_bin.h" 
...
u32 g_GestureWakeupMode[2] = {0xffff, 0};
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void) 
{
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        if (eSwId == MSG28XX_SW_ID_LIANSI)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg28xx_liansi_update_bin[127][1013]<<8 | msg28xx_liansi_update_bin[127][1012]; 
            nUpdateBinMinor = msg28xx_liansi_update_bin[127][1015]<<8 | msg28xx_liansi_update_bin[127][1014];
#else // By one dimensional array
            nUpdateBinMajor = msg28xx_liansi_update_bin[0x1FFF5]<<8 | msg28xx_liansi_update_bin[0x1FFF4];
            nUpdateBinMinor = msg28xx_liansi_update_bin[0x1FFF7]<<8 | msg28xx_liansi_update_bin[0x1FFF6];
#endif
        }
        ...
        if ((nUpdateBinMinor > nMinor && nIsCompareVersion == 1) || (nIsCompareVersion == 0))
        {
            if (eSwId == MSG28XX_SW_ID_LIANSI)
            {
                ...
                {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
                    _DrvFwCtrlStoreFirmwareData(msg28xx_liansi_update_bin[i], 1024);
#else // By one dimensional array
                    _DrvFwCtrlStoreFirmwareData(&(msg28xx_liansi_update_bin[i*1024]), 1024);
#endif
                }
            }
            else
...
void DrvIcFwCtrlGetGloveInfo(u8 *pGloveMode)
{
	...
	while (i < 5)
	{
		...
		if (rc > 0)
		{
			DBG("Get glove info IicReadData() success, szDbBusRxData[0] = %x\n",szDbBusRxData[0]);

			if (szDbBusRxData[0] == 0x0 || szDbBusRxData[0] == 0x1)
			{
				break;
			}
			else
