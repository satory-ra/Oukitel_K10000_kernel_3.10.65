...
s32 _DrvMpTestMsg26xxmItoOpenTestEntry(void)
{
    ...
    _DrvMpTestItoOpenTestMsg26xxmAnaGetMutualCSub(_gMode);
...
static void _DrvMpTestItoOpenTestAFEGainOne(voids)
{
    // AFE gain = 1X
    u16 nAfeGain = 0;
    u16 nDriOpening = 0;
    ...
    if (nDriMode == 2) // for short test
    {
        ...
        for (i = 0; i < nSF; i++)
        {
            ...
            for (j = 0; j < nAfeOpening; j++)
            {
                ...
                {
                    pResultData[i, j] = (s16)(nShotData[2 * (j + 1)] | nShotData[2 * (j + 1) + 1] << 8);                
                }
            }
        }
