...
#define DELAY_S7300_RESET_READY 180
...
#if defined(SYNAPTICS_CHARGER_SWITCH)
#define CHARGER_PLUG_IN (1 << 5)
#define CHARGER_PLUG_OUT (0 << 5)
#endif
// [gesture mode
...
static unsigned char g_log_enable = 1;
...
static void synaptics_charger_switch(struct synaptics_rmi4_data *rmi4_data);
#endif
...
void synaptics_rmi4_aw_button_down(int x, int y)
{
     struct synaptics_rmi4_data *temp_rmi4_data;

     temp_rmi4_data =exp_data.rmi4_data;

#ifdef TYPE_B_PROTOCOL
		input_mt_slot(temp_rmi4_data->input_dev, 0);
//		input_mt_report_slot_state(temp_rmi4_data->input_dev,MT_TOOL_FINGER, 1);
#endif
#if 1//def REPORT_2D_W
                        input_report_abs(temp_rmi4_data->input_dev,
                                ABS_MT_TOUCH_MAJOR, 100);
            //          input_report_abs(temp_rmi4_data->input_dev,ABS_MT_TOUCH_MINOR, min(wx, wy));
#endif

			input_report_key(temp_rmi4_data->input_dev,
					BTN_TOUCH, 1);
//			input_report_key(temp_rmi4_data->input_dev,
//					BTN_TOOL_FINGER, 1);
			input_report_abs(temp_rmi4_data->input_dev,
			...
			input_report_abs(temp_rmi4_data->input_dev,
					ABS_MT_POSITION_Y, y);
#ifndef TYPE_B_PROTOCOL
			input_mt_sync(temp_rmi4_data->input_dev);
#endif

}

 
 void synaptics_rmi4_aw_button_up(int x, int y)
 {
      struct synaptics_rmi4_data *temp_rmi4_data;
...
#ifdef TYPE_B_PROTOCOL
         input_mt_slot(temp_rmi4_data->input_dev, 0);
//         input_mt_report_slot_state(temp_rmi4_data->input_dev,MT_TOOL_FINGER, 0);
#endif
     input_report_key(temp_rmi4_data->input_dev,
             BTN_TOUCH, 0);
//     input_report_key(temp_rmi4_data->input_dev,BTN_TOOL_FINGER, 0);
#ifndef TYPE_B_PROTOCOL
     input_mt_sync(temp_rmi4_data->input_dev);
#endif
     input_sync(temp_rmi4_data->input_dev);
...
if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		if(g_log_enable)
		dev_err(&rmi4_data->i2c_client->dev,
					"%s:lfr _12_data_addr=%d, data4_off=%d, detected_gesture = %x\n",
					__func__, data_addr, extra_data->data4_offset, detected_gesture);

		
		if(detected_gesture == 2){ //if it's not double click
			synaptics_rmi4_wakeup_gesture(rmi4_data,1);
			
			if(g_log_enable)			
			dev_err(&rmi4_data->i2c_client->dev,
						"%s:lfr this is double lckil = %x\n",
        ...
        tpgesture_hander();
			...
			if(g_log_enable)			
			dev_err(&rmi4_data->i2c_client->dev,
					"%s:lfr this is flip = %x\n",
					__func__,detected_gesture);
		}
		else
		{
			dev_err(&(rmi4_data->input_dev->dev),
				"%s: read error detected_gesture!\n",
				__func__);
...
static int touch_event_handler(void *data)
{
	...
	do{
		...
		#if defined(SYNAPTICS_CHARGER_SWITCH)
		synaptics_charger_switch(rmi4_data);
		#endif
...
static void synaptics_charger_switch(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(&(rmi4_data->input_dev->dev),
				"%s: Failed to set for charger status\n",
				__func__);
		return;
	}
		charger_ctrl = (charger_ctrl  | CHARGER_PLUG_IN);
		chr_pluggedin = 1;
	 }
	 else
	 	return;
	...
	if (retval < 0) {
		...
		charger_ctrl = (charger_ctrl  | CHARGER_PLUG_OUT);
		chr_pluggedin = 0;
	 	}
	  else
	 	return;
