...
static ssize_t goodix_tool_read(struct file *flie, char __user *page, size_t size, loff_t *ppos)
{
    ...
    else if (!cmd_head.wr)
    {
        ...
        GTP_DEBUG("[CMD HEAD DATA] ADDR:0x%02x%02x.", (int)cmd_head.data[0], (int)cmd_head.data[1]);
        GTP_DEBUG("[CMD HEAD ADDR] ADDR:0x%02x%02x.", (int)cmd_head.addr[0], (int)cmd_head.addr[1]);
